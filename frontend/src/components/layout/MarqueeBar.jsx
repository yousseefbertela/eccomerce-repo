...
const MarqueeBar = ({ items }) => {
  ...
  return (
    <div className="bg-gradient-to-r from-black via-gray-900 to-black text-white py-2 overflow-hidden border-b border-accent/20">
      <div className="marquee-container">
        <div className="marquee-text text-[10px] md:text-xs uppercase tracking-[0.3em] whitespace-nowrap">
          <span className="pr-8 hover:text-accent transition-colors">{content}</span>
          <span className="pr-8 hover:text-accent transition-colors">{content}</span>
          <span className="pr-8 hover:text-accent transition-colors">{content}</span>
          <span className="pr-8 hover:text-accent transition-colors">{content}</span>
        </div>
